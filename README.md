...
📄 Paper (arXiv) | 💻 Code (Coming Soon) | 💾 Dataset
Abstract
...
Omni-Prompt Encoder: Extracts semantics from visual referents in the image to guide mask decoding. It effectively fuses prompt features with visual features using a Prompt Embed Module (PEM).
Unified Mask Decoder: A unified decoder that handles both text and visual prompts simultaneously, supporting single-target, multi-target, and no-target scenarios robustly.
OmniRef Dataset
